...
public:
    ...
    /// Call a contract (alias for callContract)
    /// @param scriptHash The contract script hash
    /// @param method The method name
    /// @param parameters The parameters
    /// @return Reference to this builder
    ScriptBuilder& contractCall(const Hash160& scriptHash, const std::string& method, const std::vector<ContractParameter>& parameters) {
        return callContract(scriptHash, method, parameters);
    }
    ...
    /// Build a multi-signature verification script from raw public key bytes
    /// @param publicKeys The public keys as raw bytes
    /// @param signingThreshold The minimum number of signatures required
    /// @return The verification script
    static Bytes buildMultiSigVerificationScript(const std::vector<Bytes>& publicKeys, int signingThreshold);
...
private:
    ...
    [[nodiscard]] uint32_t getInteropServiceHash(const std::string& method);
};
